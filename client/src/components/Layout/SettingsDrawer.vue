		...
		<div class="offcanvas offcanvas-end" tabindex="-1" id="offcanvasSettings" aria-labelledby="offcanvasSettingsLabel">
			...
			<div class="offcanvas-body">
				...
				<!-- <div v-for="(setting, key) of sett" :key="key" class="mb-3 form-check">
					<v-if v-if="setting.type === 'checkbox'">
						<input type="checkbox" v-model="setting.value" class="form-check-input" :id="setting.title.toLowerCase().replaceAll(' ', '')" />
						<label class="form-check-label" :for="setting.title.toLowerCase().replaceAll(' ', '')">{{ setting.title }}</label>
					</v-if>
				</div> -->
			</div>
		</div>

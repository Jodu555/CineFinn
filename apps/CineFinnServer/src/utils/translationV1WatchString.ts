...
import { episodesTable } from "../database.js";
...
const load = async (UUID: string): Promise<string> => {
    ...
    if (data == null || data == undefined) {
        ...
        oldDB.get('watch_strings').create(data);
    }
...
async function markSeason(UUID: string, seriesID: string, seasonID: number, bool: string | boolean) {
    ...
    if (bool) {
        ...
        const episodes = await episodesTable.get({ serie_UUID: seriesID, season_IDX: seasonID });

        segmentList.push(
            ...episodes.map((ep) => {
                return new Segment(seriesID, seasonID, ep.episode_IDX, -1, 310);
            })
        );
    } else {
        //Mark as un-watched
...
export default { Segment, generateStr, parse, load, save, updateSegment, markMovie, markSeason };

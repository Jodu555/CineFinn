...
export default class EmailManager {
  ...
  async getEmailData<T extends EmailTypes>(email_type: T, data: any): Promise<{ subject: string; html: string; text: string; }> {
    ...
    if (email_type === 'PASSWORD_RESET') {
      return this.generateEmailForgotPassword(data);
    }
  ...
  generateEmailForgotPassword(data: DataType<'PASSWORD_RESET'> & { email: string; }) {
    const { email, forgotPasswordToken } = data;
    ...
    const html = `<!DOCTYPE html>
          ...
          <body>
            <div class="email-container">
              ...
              <div class="email-body">
                <p>Hello ${email},</p>
                <p>We received a request to reset your password. To reset your password, please input the following code on the reset password page:</p>
                
                <div style="text-align: center;">
                  <h3 class="code">${forgotPasswordToken}</h3>
                </div>
    ...
    const text = `
          ...
          We received a request to reset your password. To reset your password, please input the following code on the reset password page:
    ...
    return { subject: 'Password Reeset Request', html, text };

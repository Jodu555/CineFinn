<template>
	<div class="container" v-auto-animate>
		...
		<pre v-if="authStore?.user?.settings?.developerMode?.value">
			{{ { loading, isConnected, isPlaying, error, rmvcID, socket: useSocket().id } }}
		</pre
		>
			...
			<div class="text-center col-12 col-sm-6">
				<form @submit.prevent="connect">
					<div class="mb-3">
						...
						<small id="helprmvcID" class="form-text text-secondary">The Remote Control ID that the video player gave you</small>
					</div>
				</form>
			</div>
		</div>
...
function connect() {
	...
	loading.value = true;
	error.value = '';
	useSocket().emit('rmvc-connect', { rmvcID: rmvcID.value }, ({ status }: { status: boolean }) => {
		loading.value = false;
...
onMounted(() => {
	...
	const socket = useSocket('rmvcEmitter');
	console.log(socket);

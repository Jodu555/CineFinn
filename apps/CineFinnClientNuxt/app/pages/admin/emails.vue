<template>
	<div class="container py-4">
		...
		<!-- Error State -->
		<div v-else-if="error" class="alert alert-danger">Error loading emails: {{ error }}</div>
		...
		<div v-else class="accordion accordion-flush shadow-sm" id="accordionEmails">
			<div v-for="email in emails" :key="email.UUID" class="accordion-item">
				...
				<div :id="'collapse-' + email.UUID" class="accordion-collapse collapse" data-bs-parent="#accordionEmails">
					<div class="accordion-body">
						...
						<div class="row g-3 mb-4 p-2 border rounded">
							...
							<div class="col-md-4">
								...
								<span class="small">{{ email.sent_at ? formatDate(email.sent_at) : 'Not sent yet' }}</span>
							</div>
							<div class="col-md-4">
						...
						<div class="tab-content" id="myTabContent-{{ email.UUID }}">
							...
							<div class="tab-pane fade show active" :id="'html-pane-' + email.UUID" role="tabpanel">
								<div class="card border">
									<div class="card-body p-0 overflow-auto" style="max-height: 600px">
										...
										<iframe :srcdoc="email.html" class="w-100 border-0" style="height: 600px; min-height: 400px"></iframe>
									</div>
								</div>
							</div>
							...
							<div class="tab-pane fade" :id="'text-pane-' + email.UUID" role="tabpanel">
								<pre class="p-3 border rounded" style="white-space: pre-wrap">{{ email.text || 'No text content available' }}</pre>
							</div>

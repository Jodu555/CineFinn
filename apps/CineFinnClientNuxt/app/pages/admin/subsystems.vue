<template>
	<div>
		...
		<div class="d-flex justify-content-center">
			...
			<button type="button" class="btn btn-outline-warning me-4" @click="adminStore.moveAllMovingItems()">
				Move All ({{ adminStore.movingItems.length }})
			</button>
			<button type="button" class="btn btn-outline-warning me-4" @click="adminStore.moveAllAdditionalMovingItems()">
				Move Additional ({{ adminStore.movingItems.filter((x) => x.meta.isAdditional).length }})
			</button>
			<button type="button" class="btn btn-outline-danger" @click="adminStore.removeAdditionalMovingItems()">Remove Additional Items</button>
		</div>
		<hr />
			...
			<div class="col-auto ms-5 me-auto">
				...
				<div class="d-flex gap-3 mb-3">
					<template v-if="item.meta.movingStarted == 0">
						<button
							v-if="!enqueuedMovingItems.has(item.ID)"
							@click="adminStore.moveMovingItem(item.ID)"
							type="button"

<template>
	<div class="container" v-auto-animate>
		...
		<div class="d-flex justify-content-between">
			<button v-if="authStore.user.role >= 2" class="btn btn-outline-primary mb-5" @click="addEmptyItem">Add Item</button>

			<div>
				<h2 class="text-center">AniDB</h2>
			</div>

			<button v-if="authStore.user.role >= 2" class="btn btn-outline-danger mb-5" @click="rescrapeAllItems">Rescrape All Items</button>
		</div>
							...
							<div class="card-body">
								...
								<div class="d-flex justify-content-between">
									<div class="d-flex justify-content-end gap-2">
										<small @click="refetchAnime(anime.ID)" style="cursor: pointer" class="text-success-emphasis">Refetch</small>
										<small @click="deleteAnime(anime.ID)" style="cursor: pointer" class="text-danger-emphasis">Delete</small>
									</div>
									<p class="card-text">
...
function transformAniDBAnime(data: AniDBAnime): AniDBAnime {
	...
	data.coverImage = useAPIURL() + url.pathname + '?auth-token=' + authStore.authToken;
...
async function load() {
	...
	const { data, error } = await tryCatch(() =>
		$fetch<number[]>(useAPIURL() + '/anidb/list', {
			method: 'GET',
			headers: {
	...
	for (const id of ids.value) {
		const { data, error } = await tryCatch(() =>
			$fetch<AniDBAnime>(useAPIURL() + `/anidb/anime/${id}`, {
				method: 'GET',
				headers: {
					'auth-token': authStore.authToken,
				},
		...
		const anime = transformAniDBAnime(data);
		if (!target) return;
...
async function addEmptyItem() {
	const { value: anidbID } = await $swal.fire({
	...
	if (anidbID) {
		...
		const { data, error } = await tryCatch(() =>
			$fetch<AniDBAnime>(useAPIURL() + `/anidb/anime/${anidbID}`, {
				method: 'GET',
				headers: {
					'auth-token': authStore.authToken,
				},
		...
		if (error) {
			...
			$swal.fire({
				...
				timer: 3000,
				icon: 'error',
				title: `Error loading AniDB Anime ${anidbID}!`,
				timerProgressBar: true,
			});
			return;
		}
		animes.value.push(transformAniDBAnime(data));
	}
}
...
async function refetchAnime(anidbID: number) {
	...
	const { data, error } = await tryCatch(() =>
		$fetch<AniDBAnime>(useAPIURL() + `/anidb/refetch/${anidbID}`, {
			method: 'GET',
			headers: {
				'auth-token': authStore.authToken,
			},
	...
	if (error) {
		...
		$swal.fire({
			...
			timer: 3000,
			icon: 'error',
			title: `Error refetching AniDB Anime ${anidbID}!`,
			timerProgressBar: true,
		});
		return;
	}
...
async function deleteAnime(anidbID: number) {
	...
	const { data, error } = await tryCatch(() =>
		$fetch<AniDBAnime>(useAPIURL() + `/anidb/anime/${anidbID}/delete`, {
			method: 'GET',
			headers: {
				'auth-token': authStore.authToken,
			},
	...
	if (error) {
		...
		$swal.fire({
			...
			timer: 3000,
			icon: 'error',
			title: `Error deleting AniDB Anime ${anidbID}!`,
			timerProgressBar: true,
		});
		return;
	}
...
async function rescrapeAllItems() {
	...
	const { data, error } = await tryCatch(() =>
		$fetch<AniDBAnime>(useAPIURL() + `/anidb/refetch/`, {
			method: 'GET',
			headers: {
				'auth-token': authStore.authToken,
			},
	...
	if (error) {
		...
		$swal.fire({
			...
			timer: 3000,
			icon: 'error',
			title: `Error refetching AniDB Anime List!`,
			timerProgressBar: true,
		});
		return;
	}

								...
								<div v-if="activeTab === 'seasons' && hasSeasons" class="tab-pane fade show active">
									<div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3 mb-4">
										...
										<div class="d-flex flex-column flex-md-row align-items-start align-items-md-center gap-3">
											...
											<select v-model="selectedSeason" class="form-select" style="width: auto" aria-label="Change Seasons">
												...
												<option
													v-if="additionalList != undefined"
													v-for="item in new Set(
														additionalList
															.map((e) => e.parsed.season)
															.filter((x) => x != undefined && series!.seasons.find((s) => s.season_IDX == x) == undefined),
													)"
													class="border-danger text-danger-emphasis"
													disabled
													:value="item"
												>
													Missing Season {{ item }} ({{ additionalList.map((e) => e.parsed.season).filter((x) => x == item).length }} episodes)
												</option>
											</select>
										</div>
									...
									<div v-if="viewMode === 'list'" class="d-flex flex-column gap-2">
										...
										<EpisodeListViewCard
											...
											v-for="additionalItem in additionalList.filter(
												(x) =>
...
import SeriesInfo from '~/components/SID/SeriesInfo.vue';
import EntityActionsInformation from '~/components/SID/EntityActionsInformation.vue';
import EpisodeListViewCard from '~/components/SID/EpisodeListViewCard.vue';

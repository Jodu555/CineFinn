				...
				<div style="width: 100%" ref="parent">
					<div class="d-flex justify-content-between">
						<div>
							...
							<button
								v-if="authStore.user.role >= 2 && !element.edited"
								title="Edit"
								type="button"
					...
					<ul v-if="!minimal && languageDevisionC.total != -1">
						...
						<li>
							&nbsp;&nbsp;&nbsp;&nbsp;Apx Size on Disk:
							{{ numWithFP((languageDevisionC.total * constants.mbperEpisode) / 1024, 1) }}GB
						</li>
					...
					<div v-if="!props.minimal" v-for="[key, scrapeInfo] in Object.entries(element.scrapingInfo! || {})">
						<div v-if="scrapeInfo != undefined">
							...
							<span v-if="scrapeInfo?.state === 'error'" class="h6 text-danger mb-0">
								<span
									>!!! {{ scrapeInfo.message }} !!! &nbsp;&nbsp;&nbsp;&nbsp;
									<small style="cursor: pointer" @click="todoStore.deleteOrRetryScrapeTodo(element.ID, scrapeInfo.key)">
						...
						<div v-else>This should not happen</div>
					</div>
						...
						<div>
							...
							<template v-if="authStore.user.role == 2">
								<div class="row text-center mt-2 mb-2 align-items-center">
									...
									<div class="col-1 h5">
										<span>{{ permittedAccounts.find((x) => x.UUID == element.creator)?.username }}</span>
									</div>
								</div>
							</template>
							<template v-else>
						...
						<div>
							<div v-for="refCategorie in categories">
								...
								<div
									...
									<div class="col-7">
										<input-validator
											v-model="element.refs[refkey as keyof TodoReferences]"
											v-model:valid="valid[refkey as keyof TodoReferences]"
											type="text"
											id="url"
											:name="'ref-' + refkey"
											:autocomplete="'ref' + refkey"
...
const hasMovies = computed(() => {
	if (props.element.scrapingInfo == undefined) return false;
...
const rules = reactive<Record<keyof TodoReferences, ((v: string) => string | true)[]>>(
	{} as Record<keyof TodoReferences, ((v: string) => string | true)[]>,
);

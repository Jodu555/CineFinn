        ...
        data.map(async (serie) => {
            return limit(() => {
                return new Promise<AniWorldSerieCompare>(async (res, _) => {
                    if (typeof serie.refs.aniworld == 'string') {
                        const world = new Aniworld(serie.refs.aniworld);
...
//     const data = series.filter((x) => x.refs?.aniworld && !ignoranceList.find((v) => v.ID == x.ID && !v.lang));

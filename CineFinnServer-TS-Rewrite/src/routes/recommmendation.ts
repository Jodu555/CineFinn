...
router.get('/', async (req: AuthenticatedRequest, res: Response) => {
	...
	const series = await asyncFilter(
		remoteSeries
	...
	watchagain = out
		...
		.filter((x) => x != null)
		.slice(0, watchAgainItems * 3)
		.sort((a, b) => 0.5 - Math.random())
		.slice(0, watchAgainItems);
	console.timeEnd('watchagain');

	console.time('continueWatching');
	continueWatching = out
		.slice()
		...
		.filter((x) => x != null)
		.slice(0, continueWatchingItems * 3)
		.sort((a, b) => 0.5 - Math.random())
		.slice(0, continueWatchingItems);

<template>
	<main class="container-centered">
		...
		<Carousel :settings="settings" :wrap-around="Boolean(wrapAround)" :breakpoints="breakpoints">
			<Slide v-for="(serie, index) in list" :key="serie.ID">
				<div class="carousel__item" @click="activeIdx == index ? (activeIdx = -1) : (activeIdx = index)">
					<AppCard :searchItem="serie" :isActive="activeIdx == index"></AppCard>
				</div>
			</Slide>
...
export default defineComponent({
	...
	props: {
		...
		list: { type: Array as PropType<BrowseSerie[]>, required: true },

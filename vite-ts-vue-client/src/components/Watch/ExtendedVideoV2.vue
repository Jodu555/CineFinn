    ...
    <div>
        ...
        <div style="margin-top: 0.5%" class="video-container paused" data-volume-level="high" :class="{
            ...
            <div v-show="!dataLoading" class="video-controls-container">
                <!-- Mobile-friendly seekbar -->
                <div class="timeline-container mobile-timeline-container" @mousemove="handleTimelineUpdate"
                    @mousedown="toggleScrubbing" @mouseup="toggleScrubbing" @touchstart.passive="onTimelineTouchStart"
                    @touchmove.passive="onTimelineTouchMove" @touchend.passive="onTimelineTouchEnd">
                    <div class="timeline">
                    ...
                    <!-- Add a larger invisible input range for mobile touch accessibility -->
                    <input class="timeline-range" type="range" min="0" :max="videoData.duration || 1" step="0.01"
                        v-model.number="videoData.currentTime" @input="onTimelineRangeInput"
                        @change="onTimelineRangeChange" aria-label="Seek video" />
                </div>
                <div class="controls">
...
/**
 * Mobile touch handlers for the timeline seekbar
 */
...
function onTimelineTouchEnd(e: TouchEvent) {
    ...
    handleTimelineTouch(e, true);
}
function handleTimelineTouch(e: TouchEvent, end = false) {
    if (!mainVid.value || !canPlay) return;
    const timelineContainer = (e.currentTarget as HTMLElement);
    const rect = timelineContainer.getBoundingClientRect();
    const touch = e.touches[0] || e.changedTouches[0];
    if (!touch) return;
    const percent = Math.min(Math.max(0, touch.clientX - rect.x), rect.width) / rect.width;
    previewTime.value = formatDuration(percent * mainVid.value.duration);
    const previewImgNumber = Math.max(1, Math.floor((percent * mainVid.value.duration) / 10));
    previewImgSrc.value = generatePreviewImageURL(previewImgNumber);
    timelineContainer.style.setProperty('--preview-position', String(percent));
    if (end) {
        mainVid.value.currentTime = percent * mainVid.value.duration;
        if (mainVid.value.paused && !wasPaused) mainVid.value.play();
    } else {
        thumbnailSrc.value = previewImgSrc.value;
        timelineContainer.style.setProperty('--progress-position', String(percent));
    }
...
/**
 * Handler for the input range (seekbar) for accessibility and mobile
 */
function onTimelineRangeInput(e: Event) {
    if (!mainVid.value) return;
    const target = e.target as HTMLInputElement;
    const value = parseFloat(target.value);
    mainVid.value.currentTime = value;
    updateVueVideoData();
}
function onTimelineRangeChange(e: Event) {
    if (!mainVid.value) return;
    const target = e.target as HTMLInputElement;
    const value = parseFloat(target.value);
    mainVid.value.currentTime = value;
...
// ...rest of your script setup code...
</script>

<style scoped>
/* ...existing styles... */

/* Make the timeline container easier to hit on mobile */
...
/* Make the timeline bar itself thicker for touch */
.mobile-timeline-container .timeline {
...
/* Style the input range for accessibility and hide the default thumb visually but keep it accessible */
.timeline-range {
    position: absolute;
    left: 0;
    top: 0;
    ...
    z-index: 100;
    cursor: pointer;
    /* Make sure it's above the timeline for touch */
}

/* Increase thumb size for mobile (for accessibility, but visually hidden) */

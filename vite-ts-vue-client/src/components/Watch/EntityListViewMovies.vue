<template>
	<div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-xxl-3 mt-5 g-4">
		<div class="col" v-for="(movie, idx) in currentSeries.movies">
			<div @click="changeMovie(idx + 1)" class="card text-start movie-card" :class="{
				active: Boolean(
				...
				<div class="card-body">
					...
					<p class="card-text text-secondary">
						<img v-for="lang in movie.langs" :key="lang" class="shadow mb-2 mt-3 me-2 bg-body"
							style="width: 40px" :src="`/flag-langs/${lang.toLowerCase()}.svg`"

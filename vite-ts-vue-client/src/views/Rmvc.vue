				...
				<form @submit.prevent="connect">
					<div class="mb-3">
						...
						<div class="input-group">
							<input type="text" class="form-control" id="rmvcID" v-model="rmvcID"
								aria-describedby="helprmvcID" />
							<button type="submit" class="btn btn-outline-primary">Connect</button>
						</div>
						<small id="helprmvcID" class="form-text text-secondary">The Remote Control ID that the video
							player gave you</small>
					</div>
				</form>
			</div>
		</div>
		<div v-if="isConnected" class="rmvc">
			<div class="group d-flex gap-4 justify-content-evenly mb-5">
				...
				<button :title="isPlaying ? 'Pause' : 'Play'"
					@click="$socket.emit('rmvc-send-action', { rmvcID, action: isPlaying ? 'pause' : 'play' })">
			...
			<div class="group d-flex gap-4 justify-content-evenly mt-5 mb-5">
				<button title="Vorherige Episode"
					@click="$socket.emit('rmvc-send-action', { rmvcID, action: 'prevEp' })">
			...
			<div class="row justify-content-center mt-5">
				<button type="button" @click="isConnected = false"
					class="col-8 col-sm-3 btn btn-outline-danger">Disconnect</button>
			</div>
		</div>
	</div>
</template>

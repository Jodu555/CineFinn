				...
				:class="{
					active: Boolean(
						watchList.find((segment) => {
							return segment.ID == this.$route.query.id && segment.watched && this.currentSeason == -1 && segment.movie == idx + 1;
						})
					),
				}"
			>
...
export default {
	...
	computed: {
		...mapState('watch', ['currentSeries', 'currentMovie', 'currentSeason', 'currentLanguage', 'watchList']),
	},
};
</script>
<style>

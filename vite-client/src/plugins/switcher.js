...
function deepswitchTo(vel, vueInstance) {
	if (vueInstance.currentSeason == -1) {
		...
		//Switch in Episodes
		const seasonIdx = vueInstance.currentSeries.seasons.findIndex((x) => x[0].season == vueInstance.entityObject.season);
		const episodeIdx = vueInstance.currentSeries.seasons[seasonIdx].findIndex((x) => x.episode == vueInstance.entityObject.episode);

...
const work = () => {
    ...
    const toScrollToSerie = useIndexStore().series.find((i) => i.UUID == showScrollToLastSeries.value);
...
const actualScrollIntoView = (element: HTMLElement) => {
    if (!element) return;
...
const map = (value: number, oldRange: number[], newRange: number[]) => {
